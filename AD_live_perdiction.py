...
def cd3_extartion(
    ...
    dir_list = []
    for i_batch, (data, target, sampled_idx, dirs, vid_names) in tqdm(
        ...
        with torch.no_grad():
            ...
            for i, (dir, vid_name, start_frame) in enumerate(
                zip(dirs, vid_names, sampled_idx.cpu().numpy())
            ):
...
def AD_perdiction(model_dir, dir_list, features_dir, video_parth, device=None):
    ...
    annotation_path = annotatate_file(
        video_parth, dir_list, normal=True, file_name="Demo_anmotation"
    )

    # runing vedio in to network
    data_loader = FeaturesLoaderVal(
        features_path=features_dir, annotation_path=annotation_path
    )
...
def testing(clip_size=16, video_input=0):
    ...
    device, c3d_network_n, ad_net = network_setup(
        ad_model_dir="/home/peter/Documents/actionID/AnomalyDetectionCVPR2018-Pytorch-master/short_60_low_mem/exps/model"
    )

    # C3D
    dir_list, c3d_network, data_batch = cd3_extartion(
        os.getcwd() + "/" + "test_output.avi",
        device=device,
        features_dir="./test",
        clip_length=clip_size,
    ...
    batch_y_pred = AD_perdiction(
        ad_model_dir, dir_list, features_dir, video_parth, device=device
    )
    print("batch_y_pred =" + str(batch_y_pred))

    sigle_y_pred = AD_sigle_perdiction(
        ad_model_dir, avg_segments, device=device, lengths=16, network=ad_net
    )
