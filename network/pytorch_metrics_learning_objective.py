...
class PytorchMetricLearningObjectiveWithSampling(nn.Module):
    def __init__(self, lambdas=8e-5, top_anomaly_frames=3, top_normal_frames=3, loss_name='TripletMarginLoss',
    ...
    def forward(self, normalized_embeddings, y_true):
        ...
        d_to_anomalies = (anomal_segments_embeddings - anchors).pow(2).sum(-1)  # (batch/2, segm_len, embed_dim)
        tp_frame_inds = torch.argsort(d_to_anomalies, dim=1)[:,
                        -self.top_anomaly_frames:]  # (batch/2,top_anomaly_frames)
        ...
        fp_frame_inds = torch.cdist(normal_segments_embeddings, anchors).argsort(dim=1)[:, -self.top_normal_frames:, 0]
        ...
        """
        embeddings = torch.cat([positive_frames.reshape(-1, embed_dim),
                                negative_frames.reshape(-1,embed_dim)])  # (bs*top_anomaly_frames + bs*top_normal_frames, embed_dim)
        labels = torch.Tensor([1] * (bs // 2 * self.top_anomaly_frames) + [0] * (bs // 2 * self.top_normal_frames)).to(
            embeddings.device)
