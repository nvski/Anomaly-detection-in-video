...
def triplet_objective(normalized_embeddings, y_true):
    ...
    normal_segments_embeddings = normalized_embeddings[normal_vids_indices].squeeze(-1)  # (batch/2, 32, embed_dim)
    anomal_segments_embeddings = normalized_embeddings[anomal_vids_indices].squeeze(-1)  # (batch/2, 32, embed_dim)
    ...
    d_to_anomalies = (anomal_segments_embeddings - anchors).pow(2).sum(-1)  # (batch/2, 32, embed_dim)
    tp_frame_inds = torch.argsort(d_to_anomalies, dim=1)[:, -top_anomaly_frames:]  # (batch/2,top_anomaly_frames)

    tp_frame_mask = torch.zeros((n_anom_video, n_frames), dtype=torch.bool).to(anomal_segments_embeddings.device)
    for row_idx in range(tp_frame_inds.shape[0]):
        for col_idx in tp_frame_inds[row_idx]:
            tp_frame_mask[row_idx, col_idx] = True

    positive_frames = anomal_segments_embeddings[tp_frame_mask].view(n_anom_video, -1,
                                                                     embed_dim)  # (batch/2,top_anomaly_frames, embed_dim)
    ...
    negative_frames = normal_segments_embeddings[fp_frame_mask].view(n_norm_video, -1,
                                                                     embed_dim)  # (batch/2,top_anomaly_frames, embed_dim)
...
def triplet_objective_sampling(normalized_embeddings, y_true, lambdas=8e-5, top_anomaly_frames=3, top_normal_frames=3, margin=0.2):
    # normalized_embeddings (batch_size, 32, embed_dim)
    # y_true (batch_size)
    ...
    positive_frames = normal_segments_embeddings[:, 1:1+top_normal_frames, :].view(n_norm_video, -1, embed_dim)
    ...
    fp_frame_inds = torch.cdist(anomal_segments_embeddings, anchors).argsort(dim=1)[:, -top_anomaly_frames:, 0]
    fp_frame_mask = torch.zeros((n_anom_video, n_frames), dtype=torch.bool).to(anomal_segments_embeddings.device)
    for row_idx in range(fp_frame_inds.shape[0]):
        for col_idx in fp_frame_inds[row_idx]:
            fp_frame_mask[row_idx, col_idx] = True

    negative_frames = anomal_segments_embeddings[fp_frame_mask].view(n_anom_video, -1,
                                                                     embed_dim)  # (batch/2,top_anomaly_frames, embed_dim)
    ...
    Smoothness of anomalous video
    """
    smoothed_scores = anomal_segments_embeddings[:, 1:] - anomal_segments_embeddings[:, :-1]
    smoothed_scores_sum_squared = smoothed_scores.pow(2).sum(dim=-1)
