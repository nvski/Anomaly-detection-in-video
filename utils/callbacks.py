...
class DefaultModelCallback(Callback):
    ...
    def __init__(self, log_every=500, visualization_dir=None):
        """
        Args:
    ...
    def on_training_end(self, model):
        if self.visualization_dir is not None:
            ...
            plt.plot(
                range(1, self.epochs + 1), self.train_losses, label=f"Training loss"
            )
            if len(self.val_loss) != 0:
                plt.plot(
                    range(1, self.epochs + 1), self.val_loss, label=f"Validation loss"
                )
...
class TensorBoardCallback(Callback):
    ...
    def on_training_iteration_end(self, train_loss, val_loss):
        if train_loss is not None:
            self.tb_writer.add_scalars(
                "Epoch loss", {f"Loss (train)": train_loss}, self.epoch
            )

        if val_loss is not None:
            self.tb_writer.add_scalars(
                "Epoch loss", {f"Loss (validation)": val_loss}, self.epoch
            )
